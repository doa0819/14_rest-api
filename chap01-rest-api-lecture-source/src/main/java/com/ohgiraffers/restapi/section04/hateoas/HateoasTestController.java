...
public class HateoasTestController {
    ...
    * */
    ...
    public HateoasTestController(){
        ...
        users.add(new UserDTO(2,"user02","pass02","루이바오", LocalDate.now()));
        users.add(new UserDTO(3,"user03","pass03","후이바오", LocalDate.now()));
    }
    ...
    /* Hateoas 적용 전체 조회 */
    @GetMapping("/users")
    public ResponseEntity<ResponseMessage> foundAllUsers(){

        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(new MediaType("application", "json", Charset.forName("UTF-8")));

        /* EntityModel : Hateoas 에서 제공하는 클래스
        *                rest api 를 만들 때 해당 리소스와 관련 된 링크를 포함 할 수 있게함
        * Rel : 링크 이름
        *     : 회원에 대한 한명한명의 정보도 주겠다! -> 회원에 대한거 한명한명을 링크로 보내준다!
        *  */
        List<EntityModel<UserDTO>> userWithRel
                = users.stream().map(user -> EntityModel.of(
                        ...
                        linkTo(methodOn(HateoasTestController.class).foundAllUsers()).withRel("users")
        )).collect(Collectors.toList());
